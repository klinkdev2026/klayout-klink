...
Implemented strictly to docs/FLEXDR_FAITHFUL_SPEC.md (cites OpenROAD source).
Build order = the spec's M1..M5; each piece verified vs the golden oracle in
tests/unit/test_flexdr.py.
...
# rust/klink_boxmaze/Cargo.toml. See docs/FLEXDR_RUST_ACCEL_PLAN.md.
...
def _attach_rust_grid(g: CapacityGrid, use_rust: bool) -> None:
    ...
    if not use_rust or _BOXMAZE_RS is None:
        return
    net_id: Dict[str, int] = {}
    nxt = 0
...
def flexpa_access_nets(g, netlist, placement, device_pads, terms, *, route_only=None,
                       wire_width_um: float = 0.0, extra_terminals=None):
        ...
        for ref in net["terminals"]:
            ...
            if not legal:
                ...
                if not legal and overlap:
                    ...
                    # ...but NEVER force-open a cell a FOREIGN net already owns
                    # (e.g. a VDD followpins rail overlapping this S/D pad): that
                    # makes the cell multi-owner, so _wire_ok is False and the
                    # route cannot even start on its own pin. Prefer a foreign-free
                    # overlapping cell; only if every one is foreign-owned (the pad
                    # is wholly under another net) fall back to the nearest.
                    ...
                    # If the fallback cell is no net's REAL pad metal -- only foreign
                    # clearance HALOs claim it (a sub-pitch pad whose access lands in
                    # a neighbour's halo) -- strip those owners so this pin is
                    # reachable. A foreign REAL pad here is a genuine conflict: leave.
            ...
            if cand:                               # nearest pad-overlapping cell to launch-edge AP
                best = min(cand, key=lambda c: abs(g.cx(c[0]) / 1000.0 - ax)
                           + abs(g.cy(c[1]) / 1000.0 - ay))
                tcells.append({best})
        # fixed off-device terminals (e.g. a pre-placed probe-card pad assigned to
        # this net): the WHOLE legal on-pad cellset is one target set, so the maze
        # may land anywhere on the pad. The pad box must be registered as this
        # net's own metal (extra_pads_by_layer) and inside the grid bbox
        # (bbox_include_um) by the caller.
        for (x1, y1, x2, y2, layer) in (extra_terminals or {}).get(nm, ()):
            li = g.layers.index(layer)
            legal = {c for c in _cells_in_box(g, x1, y1, x2, y2, li)
                     if _wire_ok(g, c[0], c[1], c[2], nm)}
            if not legal:
                w_ap = wire_width_um if wire_width_um > 0 else g.pitch_nm / 1000.0
                legal = {c for c in _cells_overlapping_pad(g, x1, y1, x2, y2, li, w_ap)
                         if _wire_ok(g, c[0], c[1], c[2], nm)}
            if not legal:
                raise ValueError(
                    f"extra terminal for net {nm!r} on {layer} at ({x1},{y1},{x2},{y2}) "
                    "has no reachable grid cell: the pad box must be inside the grid "
                    "(pass it in build_grid's bbox_include_um), registered as this "
                    "net's own metal in extra_pads_by_layer, and clear of other "
                    "nets' metal/keep-outs (a pad under a foreign halo is "
                    "multi-owner and unusable -- move the pad)")
            tcells.append(legal)
        if tcells:
...
def route_flexdr(
    ...
    *,
    ...
    _attach_rust_grid(g, use_rust)   # optional byte-parity Rust kernel (no-op if off/absent)
    ...
    # Lab-fast: greedy net-by-net seed only. The FlexTA track-assignment seed is
    # a separate multi-layer engine, not carried by this frozen module
    # (kept byte-parity: 1befa916/3e910c67).
    corridors = None
