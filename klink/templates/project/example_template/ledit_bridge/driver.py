...
def check_alive():
    if not os.path.exists(HELLO):
        raise SystemExit(
            ...
            "-> is L-Edit running with ledit_bridge64.upi loaded? "
            "(Tools > Macro > Load Macro...)")
    age = time.time() - os.path.getmtime(HELLO)
    if age > 10:
        raise SystemExit(
            f"hello.json is stale ({age:.0f}s old)\n"
            "-> bridge timer not running. In L-Edit: Tools > klink: Bridge Start")
...
def demo():
    ...
    for name, gl, rgb in [("WL", 1, [40, 110, 255]),
        ...
        print(f"ensure_layer {name} ->", call(
            "ensure_layer", {"name": name, "gds_layer": gl, "gds_datatype": 0}))
        print(f"set_layer_style {name} ->", call(
            "set_layer_style", {"layer": name, "fill_rgb": rgb}))
    print("create_cell ->", call("create_cell", {"name": "klink_demo_unit"}))
    items = [
    ...
    print("draw ->", call("draw", {"cell": "klink_demo_unit", "items": items}))
    print("create_cell ->", call("create_cell", {"name": "klink_demo_top"}))
    print("place_instance ->", call("place_instance", {
        "cell": "klink_demo_top", "child": "klink_demo_unit",
