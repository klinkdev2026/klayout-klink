...
except ImportError:  # scaffolded into a klink-init project (no examples_klink pkg)
    import os as _os, sys as _sys
    _sys.path.insert(0, _os.path.dirname(_os.path.abspath(__file__)))
    import fit_device_pnr_lvs as D
...
if not NO_CARD:
    ...
    CARD = os.path.abspath(str(D._OUT / "pub_probe_card_demo.gds"))
...
with KLinkClient(port=PORT).connect() as c:
    ...
    for p in res.get("problems", [])[:6]:
        print("   problem:", p)
    dump = os.path.abspath(str(D._OUT / f"{CELL.lower()}_dump.gds"))
