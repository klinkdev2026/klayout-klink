...
DOMAINS: "OrderedDict[str, dict]" = OrderedDict([
    ...
    ("geometry_authoring", {
        ...
        "usage": (
            ...
            "CHECKING geometry is what these are for, not hand-written "
            "rectangle math: geometry.boolean answers enclosure and overlap "
            "as an AREA (op='not' a minus b -- >0 means a is not covered by "
            "b; op='and' -- >0 means they overlap), and drc.run takes DRC DSL "
            "source INLINE, running against the loaded layout when the script "
            "omits source() (no deck file needed). Layer identity is spelled "
            "differently per tool: shape.query takes layer_index, "
            "geometry.boolean takes 'L/D' or {layer, datatype}. A truthful "
            "area is still not a verdict -- whether an overlap is a short or "
            "an intended connection is design INTENT; say a check is "
            "inconclusive rather than reporting a finding you cannot ground.\n"
    ...
    ("selection_and_send_memory", {
        ...
        "usage": (
            ...
            "SEND captures selected RULERS too (record keys ruler_count + "
            "rulers[], ascending id = creation order; each with outline, "
            "points_um, bbox_um). \"claim the rulers I sent\" = "
            "interaction.selection.latest -> rulers[].id -> region.claim; "
            "ruler ids can be reused after deletions, so cross-check "
            "points_um with annotation.get before consuming one."
        ),
    }),
    ("rulers_and_measurement", {
        ...
        "usage": (
            ...
            "to reach one — except SEND: a ruler selected when the user "
            "presses SEND lands in interaction memory (rulers[]).\n"
    ...
    ("ports_and_anchors", {
        ...
        "usage": (
            ...
            "anchor.* (+ anchor.repair_ids). port.mark_many marks many ports in "
            "one cell with a single RPC/undo step (validate-before-mutate: one "
            ...
            "999/10): the user draws RULERS around a spot — 2-point box/ellipse "
            "rulers, or 3+ point rulers read as EXACT closed polygons "
            "(auto-closed; self-intersecting outlines refused) — then "
            ...
            "rejected, claim them separately.\n"
            "PICKING the rulers is the hazard (the view mixes this-moment "
            "intent with old measurement lines). Order of resolution: (1) the "
            "user SENT rulers -> interaction.selection.latest.rulers[].id; (2) "
            "else region.claim_preview (no mutation: every ruler newest first "
            "with kind/points/bbox/label/claimable, plus a dry-run compose of "
            "a given set) -> NARRATE the candidates to the user, propose a "
            "set, claim only after they confirm — resolve by order/count, ask "
            "when ambiguous, never guess; (3) rulers labeled 'region' / "
            "'region:exclude' (labeled_role) may be taken without asking. "
            "Never auto-clear the user's rulers. claim echoes each consumed "
            "ruler (consumed[]). region.list / get / unclaim / set_layer; "
            "region.get returns the composed polygon (hull_um) — feed it to "
            "cell.fill_region to fill the area, or bbox_um to view.zoom_box "
            "to navigate there. The user can also click a Region and SEND it; "
            "resolve via interaction.selection.*.\n"
    ...
    ("device_structdevice", {
        ...
        "usage": (
            ...
            "runtime (no plugin reload); verify byte-exact first "
            "(pcell_diff.verify_differential) and pass diff_report.\n"
    ...
    ("bridge_ledit", {
        ...
        "usage": (
            ...
            "(draw/read/place/navigate/manage designs+layers+T-Cells). When the "
            ...
            "via the macro's import_gds) to put the result back. The "
            "mistake is not only rebuilding a router out of draw calls: "
            "the quieter one is reading coordinates with get_cell and "
            "reasoning about them in Python because 'it is only geometry'. "
            "Measured -- an agent doing that reported a CRITICAL DESIGN "
            "ERROR on a correct NMOS (n+ covering the gate, which is how a "
            "self-aligned device is drawn). Geometry carries PROCESS "
            "meaning: geometry.boolean/drc_run encode it, first-principles "
            "reasoning invents rules. Note a KLayout PCell lands as STATIC "
            ...
            "- ledit.push_cell_tree / ledit.import_cell_tree — the same "
            "trip for a whole cell SUBTREE, hierarchy intact: cells in "
            "dependency order, instances rebuilt as instances (arrays and "
            "orthogonal rotations included), layer identity by NAME, "
            "idempotent, one batched request. Placements L-Edit cannot "
            "express exactly (magnification, skewed array, non-orthogonal "
            "rotation) are REPORTED, never approximated. For a whole "
            "design rather than a subtree, move a GDS file instead — the "
            "macro's import_gds is faster but maps layers by NUMBER and "
            "makes parametric content static.\n"
            "- ledit.show_cell / ledit.set_cell_hidden / ledit.list_windows "
            "/ ledit.close_window / ledit.layout_view — navigation (macro "
            ">= 0.5.6): 'show me cell X', 'hide the variant cells', 'zoom "
            "to this area', close a window. layout_view is one verb: no "
            "args reads, rect_um sets, home resets; it always answers with "
            "the view after the call.\n"
            "- ledit.save_image — PNG/BMP/JPG of a cell or area. A "
            "user-requested artifact only, never agent evidence (same "
            "rule as view.screenshot).\n"
            "- ledit.delete_cell / ledit.rename_cell / ledit.delete_objects "
            "/ ledit.close_design — destructive, explicit targets only "
            "(macro >= 0.5.7): delete_cell refuses visible/instanced/"
            "T-Cell cells without force and names the parents; "
            "delete_objects by layer and/or rect_um (object fully INSIDE "
            "the rect); close_design refuses unsaved changes without "
            "discard and is the only way to drop a scratch design "
            "(closing its last window keeps it open). Confirm with the "
            "user before deleting anything they drew.\n"
            "- ledit.run_drc / ledit.drc_summary / ledit.export_gds — "
            "verification (macro >= 0.5.8): L-Edit's own DRC with the "
            "design's rule set, reporting the error COUNT and status "
            "only (L-Edit v16.3 does not expose the violation geometry "
            "through the UPI; for violation geometry use export_gds + "
            "klink's KLayout-side drc tools), and GDS export as the "
            "cheap L-Edit -> KLayout return path (read it with "
            "layout.file_info / layout.import_file; no 2048-byte "
            "padding needed in this direction).\n"
