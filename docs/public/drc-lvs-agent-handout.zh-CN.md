...
全部在 live 会话验证过。度量名:`euclidian`(默认,真最短距离含拐角)、
`projection`(只量平行边)、`square`。
...
| `.width(x.0 [, 度量])` | 最小线宽 | `m1.width(2.0, euclidian)` |
| `.space(x.0 [, 度量])` | 最小间距(含凹口) | `m1.space(2.0, projection)` |
| `.isolated(x.0)` | 只查不同多边形之间 | — |
| `.notch(x.0)` | 只查多边形内部凹口 | — |
| `a.enclosed(b, x.0)` | a 必须内缩于 b ≥ x | `cut.enclosed(m1, 0.5)` |
| `a.enclosing(b, x.0)` | a 必须外扩超过 b ≥ x | — |
| `a.separation(b, x.0 [, 选项])` | 两层间最小距离 | — |
| `a.overlap(b, x.0)` | 最小交叠深度 | — |
| `.sized(x.0)` | 区域外扩(负=内缩) | `dev = input(29,0).sized(10.0)` |
| `.sized(-w).sized(w)` | 宽特征派生(抹掉窄于 2w 的一切) | `wide = m1.sized(-1.5).sized(1.5)` |
| `wide.space(x.0, projection_limits(l.0, nil))` | 宽特征之间的间距表/平行走线长度(PRL)规则(已验证形式) | `wide.space(3.0, projection_limits(5.001, nil))` |
| `.edges.with_angle(a, absolute)` | 按角度选边 | `m1.edges.with_angle(45, absolute)` |
| `.with_length(l.0, nil)` | 按长度过滤边 | — |
| `.with_area(0, a)` | 低于最小面积的多边形 | `m1.with_area(0, 0.09)` |
| `.ongrid(g)` | 偏离制造网格的顶点 | `m1.ongrid(0.005)` |
| `.polygons` | 错误标记 → 多边形 | `errs = m1.space(2.0).polygons` |
| `.outside(区域)` | 只保留完全在区域外的标记 | `errs.outside(dev)` |
| `.output("类别", "描述")` | 违例入库归类 | — |
| `.forget` | 释放中间层(大 deck 必备) | `m1_a_l.forget` |
| `&`、`.and()`、`.not()`、`.join()`、`.xor()` | 层布尔 | `gate = active & poly` |
| `deep` / `tiles(x.um)` + `tile_borders(y.um)` + `threads(n)` / `flat` | 运行模式(deep 与 tiles 互斥) | — |
| `connect(a, b)` / `connect_global(l, "NET")` / `antenna_check(...)` | 连通性 + 天线(实践中单独 deck) | — |

## 产线 deck 骨架(照这个结构写,别写零散检查)

被要求做的不只是快查时,按发布 PDK deck(IHP SG13G2 / GF180MCU——去读)
的方式组织:
...
## 配方:完整 LVS deck(晶体管级、SPICE 参考)

用户有 SPICE 原理图和真器件时,写 KLayout `.lvs` 脚本(官方 LVS DSL),
流水线固定为:派生层 → `connect(a,b)` / `connect_global(l,"VSS")` /
`connect_implicit("VDD*")` →
`extract_devices(mos4('名'), {'SD'=>..,'G'=>..,'tS'=>..,'tD'=>..,'tG'=>..,'W'=>..})`
→ `schematic('ref.cir')` → `align` → 归约(`netlist.simplify`、
`netlist.combine_devices`、`netlist.purge`、`max_res(1e9)`、`min_caps(1e-18)`)
→ `success = compare`(+ `flag_missing_ports`)→ `report_lvs(路径)`。
逃生口:`tolerance(器件, 参数, ...)`、`same_nets`、`equivalent_pins`、
`blank_circuit('IP_*')` 黑盒。提取器类:mos3/mos4、dmos3/dmos4、
`resistor(名, 方阻)`、`capacitor(名, 面积电容)`、diode、bjt3/bjt4。不要
发明别的;拿不准时先读一个发布 deck(IHP sg13g2.lvs)再动笔。

## 配方:klink LVS(声明网,布线流程的裁判)
