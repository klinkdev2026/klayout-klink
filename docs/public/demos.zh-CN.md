...
(`nanodevice/`、`photonics/`、`digital/`、`passives/`、`layout/`)。没有一个需要你的
...
KLayout 自带 Fill Utility 的 RPC 形态(`cell.fill_region`)。在一个演示 cell
里把 2×1 µm 的填充 cell 平铺进五种区域:草稿层上手绘的多边形直接当区域
(`region_layers`)、整圆、90° 扇形、带缝矩形阵列、疏/密同圆对照
(间距 = 步距 − 器件脚印,x/y 独立)。只放完整落在区域内的 tile;每次调用
把未覆盖的边缘如实报为 `remaining_area_um2`,打印表自校验:
tile 数 × 脚印面积 + remaining == 区域面积。实测:手绘 253、整圆 187、
