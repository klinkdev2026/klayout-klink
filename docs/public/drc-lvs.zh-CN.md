...
这套验证闭环已经赚回过一次成本:它抓到一个在 KLayout 0.30.x 上"能解析
但永不触发"的文档化 PRL idiom(完整排障过程与调试矩阵见
[issue #1](https://github.com/klinkdev2026/klayout-klink/issues/1);
已验证形式见 §2.5)。配方便宜到指南、手册或你自己的 deck 每次改动都值得
重跑:
