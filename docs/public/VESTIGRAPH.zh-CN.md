...
一次连续的 AI 绘图操作会合并为一个检查点。AI 写入前会先把待处理的手动修改保存为检查点。`vestigraph.history` 默认返回最近 30 条摘要，只在用户明确要求时读取全部。

用户选定检查点后，用当前已保存的文档/会话和简短原因调用 `vestigraph.restore`。恢复会替换工作文件并重新载入，然后追加带恢复来源的新检查点。它不会删除任何更早或更晚的检查点。
